...
def cls_loss(logits, labels, weight=1):
    ...
    :return:
    """
    return tf.compat.v1.losses.sparse_softmax_cross_entropy(logits=logits, labels=tf.compat.v1.to_int32(labels),
                                                  weights=weight)
...
def smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    sign = tf.stop_gradient(tf.compat.v1.to_float(tf.less(abs_in_box_diff, 1. / sigma_2)))
